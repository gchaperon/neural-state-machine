...
class DataShell(QuitCmd):
    ...
    def _launch_graph_shell(self, example, index):
    ...
    def do_show(self, arg):
        ...
        self._launch_graph_shell(example, index=i)
        self.hist.append(i)

    def do_adversarial(self, arg):
        n_impostors = int(arg or 10)
        example = self.dataset.random_adversarial(n_impostors)
        # breakpoint()
        self._launch_graph_shell(example, index="adversarial")
        pass
    
    do_adv = do_adversarial
...
def run(args):
    ...
    # breakpoint()
    datamodule = ClevrNoImagesDataModule("data", batch_size=1, w_instructions=True)
